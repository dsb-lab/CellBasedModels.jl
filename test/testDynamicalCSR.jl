@testset "DynamicalCSR" begin
    ...
    for backend in backends
        ...
        function _dcsr_iterate_row_sum(csr, row_idx, result)
            @kernel function iter_kernel!(csr, row_idx, result)
                iter = CellBasedModels.iterateRow(csr, row_idx)
                sum_val = 0.0
                count = 0
            ...
            backend = KernelAbstractions.get_backend(csr)
            threads = backend === CPU() ? 1 : 256
            iter_kernel!(backend, threads)(csr, row_idx, result, ndrange=1)
            KernelAbstractions.synchronize(backend)
        end
        
        csr_iter = dcsr_zeros(Float64, 3, 3, 2)
        csr_iter = toBackend(backend, csr_iter)
        _dcsr_slice_setup(csr_iter)  # Sets up: [1,1]=1, [1,2]=2, [2,1]=3, [2,3]=4, [3,2]=5
        
        result = toBackend(backend, zeros(Float64, 2))
        _dcsr_iterate_row_sum(csr_iter, 1, result)
        @test Array(result)[1] == 3.0  # 1.0 + 2.0 (row 1 values)
        @test Array(result)[2] == 2.0  # 2 entries in row 1
        
        result = toBackend(backend, zeros(Float64, 2))
        _dcsr_iterate_row_sum(csr_iter, 2, result)
        @test Array(result)[1] == 7.0  # 3.0 + 4.0 (row 2 values)
        @test Array(result)[2] == 2.0  # 2 entries in row 2
        
        result = toBackend(backend, zeros(Float64, 2))
        _dcsr_iterate_row_sum(csr_iter, 3, result)
        @test Array(result)[1] == 5.0  # 5.0 (row 3 values)
        @test Array(result)[2] == 1.0  # 1 entry in row 3
        
        # Test iterator length
        iter_cpu = CellBasedModels.iterateRow(toBackend(CPU(), dcsr_zeros(Float64, 3, 3, 0)), 1)
        @test length(iter_cpu) == 3  # 3 slots per row
        
        # Test out-of-bounds row returns empty iterator
        iter_oob = CellBasedModels.iterateRow(toBackend(CPU(), dcsr_zeros(Float64, 3, 3, 0)), 10)
