@testset "DynamicalELL" begin
    ...
    function _dell_replace_same_row(ell, result, i, j_old, j_new)
        @kernel function replace_kernel!(ell, result, i, j_old, j_new)
            result[1] = CellBasedModels.replaceIndex!(ell, i, j_old, j_new) ? 1 : 0
        end
        backend = KernelAbstractions.get_backend(ell)
        threads = backend === CPU() ? 1 : 256
        replace_kernel!(backend, threads)(ell, result, i, j_old, j_new, ndrange=1)
        KernelAbstractions.synchronize(backend)
    end

    function _dell_replace_cross_row(ell, result, i_old, j_old, i_new, j_new)
        @kernel function replace_kernel!(ell, result, i_old, j_old, i_new, j_new)
            result[1] = CellBasedModels.replaceIndex!(ell, i_old, j_old, i_new, j_new) ? 1 : 0
        end
        backend = KernelAbstractions.get_backend(ell)
        threads = backend === CPU() ? 1 : 256
        replace_kernel!(backend, threads)(ell, result, i_old, j_old, i_new, j_new, ndrange=1)
        KernelAbstractions.synchronize(backend)
    end
    ...
    for backend in backends
        ...
        _dell_replace_same_row(ell_replace, result_arr, 1, 1, 5)
        ...
        result_arr = toBackend(backend, [0])
        _dell_replace_cross_row(ell_replace, result_arr, 2, 1, 3, 7)
        @test Array(result_arr)[1] == 1
        ...
        result_arr = toBackend(backend, [0])
        _dell_replace_cross_row(ell_replace, result_arr, 99, 99, 100, 100)
        ...
        ell_dense = dell_zeros(Float64, 3, 3, 2)
        ell_dense = toBackend(backend, ell_dense)
        _dell_slice_setup(ell_dense)  # Sets up: [1,1]=1, [1,2]=2, [2,1]=3, [2,3]=4, [3,2]=5
        dense = CellBasedModels.todense(ell_dense)
        @test size(dense) == (3, 3)
        @test dense[1, 1] == 1.0
        @test dense[1, 2] == 2.0
        @test dense[2, 1] == 3.0
