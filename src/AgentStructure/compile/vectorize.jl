...
"""
    function vectorize_(abm::Agent,code::Expr,p::Agent;interaction=false)
...
"""
function vectorize(code::Expr,abm::Agent;interaction=false,integrator=Euler,integrationStep::Int=1,variablesUpdated=All)
    ...
    for (i,v) in enumerate(abm.declaredSymbols[abm.declaredSymbols[:,:scope].==:Local,:name])

        vnew = Meta.parse(string(v,:Copy))

        code = postwalk(x->@capture(x, vAux_.i.new) && vAux == v ? :($vnew[ic1_]) : x, code)
        code = postwalk(x->@capture(x, vAux_.j.new) && vAux == v ? :($vnew[nnic2_]) : x, code)
        code = postwalk(x->@capture(x, vAux_.i) && vAux == v ? :($v[ic1_]) : x, code)
        code = postwalk(x->@capture(x, vAux_.j) && vAux == v ? :($v[nnic2_]) : x, code)
        if interaction
            code = postwalk(x->@capture(x, vAux_) && vAux == v ? errorInteraction(vAux) : x, code)
        else
            code = postwalk(x->@capture(x, vAux_.new) && vAux == v ? :($v[ic1_]) : x, code)
            code = postwalk(x->@capture(x, vAux_) && vAux == v ? :($v[ic1_]) : x, code)
        end

    end

    for (i,v) in enumerate(abm.declaredSymbols[abm.declaredSymbols[:,:scope].==:Global,:name])

        vnew = Meta.parse(string(v,:Copy))

        code = postwalk(x->@capture(x, vAux_.new) && vAux == v ? :($vnew[1]) : x, code)
        code = postwalk(x->@capture(x, vAux_) && vAux == v ? :($v[1]) : x, code)

    end

    for (i,v) in enumerate(abm.declaredSymbols[abm.declaredSymbols[:,:scope].==:Medium,:name])
...
"""
    function vectorizeMedium_(abm::Agent,code::Union{Expr,Symbol,<:Number},p::Agent)
