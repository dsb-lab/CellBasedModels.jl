"""
    function addIntegratorMediumFTCS_!(p::AgentCompiled,platform::String)

Generate the functions related with Medium Update with FTCS integration method.

This method is conditionally stable for advection functions without viscosity (ecc. functions with the form `∂ₜx(x) = ∇x u(x)`). It is only advised to use with difussion equations. 
