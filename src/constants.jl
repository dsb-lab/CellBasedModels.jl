...
#Parameters of simulation topology
    :simBox                       => BaseParameter(:Float,   (:Dims,2),                 0,          :Base,        false,       false,     false,    Symbol[:Medium,:CellLinked],          @eval (com,agent) -> Float64[0. 1;0 1;0 1][1:agent.dims,:]                                            ),
...
#Parameters of neighborhoods
    :skin                         => BaseParameter(:Float,   (:Global,),                0,          :Neighbor,    false,       false,     false,    Symbol[:VerletTime,:VerletDisplacement],  @eval (com,agent) -> Float64[0.]                                                                 ),
    :dtNeighborRecompute          => BaseParameter(:Float,   (:Global,),                0,          :Neighbor,    false,       false,     false,    Symbol[:VerletTime],                  @eval (com,agent) -> Float64[0.]                                                                 ),
    :nMaxNeighbors                => BaseParameter(:Int,     (:Global,),                0,          :Neighbor,    false,       false,     false,    Symbol[:VerletTime,:VerletDisplacement],  @eval (com,agent) -> Int[0]                                                                    ),
    :cellEdge                     => BaseParameter(:Float,   (:Dims,),                  0,          :Neighbor,    false,       false,     false,    Symbol[:CellLinked],                  @eval (com,agent) -> Float64[1.,1.,1.][1:agent.dims]                                                                 ),
    ...
    :neighborN_                   => BaseParameter(:Int,     (:Local,),                 2,          :Neighbor,    true ,       true,      false,    Symbol[],                             @eval (com,agent) -> if agent.neighbors in [:VerletDisplacement,:VerletTime]; zeros(Int64,com[:N][1]) else zeros(Int64,0); end                                                      ),
    :neighborList_                => BaseParameter(:Int,     (:Local,:Neighbors),       2,          :Neighbor,    true ,       true,      false,    Symbol[],                             @eval (com,agent) -> if agent.neighbors in [:VerletDisplacement,:VerletTime]; zeros(Int64,com[:N][1],com[:nMaxNeighbors][1]) else zeros(Int64,0,0); end                                  ),
    :neighborTimeLastRecompute_   => BaseParameter(:Float,   (:Global,),                2,          :Neighbor,    false,       true,      false,    Symbol[],                             @eval (com,agent) -> Float64[0]                                                                ),
    :posOld_                      => BaseParameter(:Float,   (:Local,:Dims),            2,          :Neighbor,    true,        true,      false,    Symbol[],                             @eval (com,agent) -> if agent.neighbors == :VerletDisplacement; zeros(Float64,com[:N][1],agent.dims) else zeros(Float64,0,agent.dims); end                                          ),
    :accumulatedDistance_         => BaseParameter(:Float,   (:Local,),                 2,          :Neighbor,    true,        true,      false,    Symbol[],                             @eval (com,agent) -> if agent.neighbors == :VerletDisplacement; zeros(Int64,com[:N][1]); else zeros(Int64,0); end                                                      ),
    :nCells_                      => BaseParameter(:Int,     (:Dims,),                  2,          :Neighbor,    false,       true,      false,    Symbol[],                             @eval (com,agent) -> if agent.dims == 0 || !(agent.neighbors in [:CellLinked]); Int64[0]; else ceil.(Int64,(com[:simBox][:,2].-com[:simBox][:,1])./com[:cellEdge][1] .+2); end        ),
