...
module IntegrationAlgs
    ...
    # Helper function to fill arrays with random normal values
    # Works with both regular arrays and UnstructuredMeshObjects
    function fill_randn!(arr)
        if arr isa Array
            Random.randn!(arr)
        else
            # Use recursivefill! for mesh objects
            RecursiveArrayTools.recursivefill!(arr, randn)
        end
    end
...
"""
    ...
    function DifferentialEquations.step!(obj::EulerHeun,dt=obj.dt,_=nothing)
        ...
        if typeof(obj.u) <: Array
            ...
            fill_randn!(obj.rand)
            obj.u .= obj.u .+ ( ( obj.du_f1 .+ obj.du_f2 ) .*dt .+ ( obj.du_g1 .+ obj.du_g2 ) .* obj.rand .* sqrt(dt) ) ./ 2
        end
...
"""
    ...
    function DifferentialEquations.step!(obj::DGADI,dt=obj.dt,_=nothing)
        ...
        if typeof(obj.du) <: Array
            ...
            # GPU CUDA code - disabled for now, CUDA should be handled via extension
            error("DGADI GPU support is not yet available in this version. Use CPU arrays only.")
            #= 
    ...
    function init(problem::SciMLBase.AbstractDEProblem,alg::CustomIntegrator;kwargs...)
        ...
        if typeof(alg) <: Euler
            ...
            if typeof(problem) <: ODEProblem
                ...
                error("Euler algorithm is for ODE problems only, a SDE problem has been detected. Choose an appropiate integrator from IntegrationAlgs or ")
            end
        ...
        elseif typeof(alg) <: Heun
            ...
            if typeof(problem) <: ODEProblem
                ...
                error("Heun algorithm is for ODE problems only, a SDE problem has been detected. Choose an appropiate integrator from IntegrationAlgs or ")
            end
        ...
        elseif typeof(alg) <: RungeKutta4
            ...
            if typeof(problem) <: ODEProblem
                ...
                error("RungeKutta4 algorithm is for ODE problems only, a SDE problem has been detected. Choose an appropiate integrator from IntegrationAlgs or ")
            end
        ...
        elseif typeof(alg) <: EM
            ...
            if typeof(problem) <: SDEProblem
                ...
                error("EM algorithm is for ODE problems only, a SDE problem has been detected. Choose an appropiate integrator from IntegrationAlgs or ")
            end
        ...
        elseif typeof(alg) <: EulerHeun
            ...
            if typeof(problem) <: SDEProblem
                ...
                error("EulerHeun algorithm is for ODE problems only, a SDE problem has been detected. Choose an appropiate integrator from IntegrationAlgs or ")
            end
        ...
        elseif typeof(alg) <: DGADI
            ...
            if typeof(problem) <: ODEProblem
                ...
                error("DGADI algorithm is for PDE problems only, a SDE problem has been detected. Choose an appropiate integrator from IntegrationAlgs or ")
            end
